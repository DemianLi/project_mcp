...
>
> **憑證那一項有答案了**（2026-09-10，讀 `gh` 2.91.0 釘的 `go-gh` v2.13.0，
> `pkg/auth/auth.go` 的 `tokenForHost`）：**看部署設哪個變數**。
>
> | 部署設的憑證 | 呼叫方指定的主機收到什麼 |
> |---|---|
> | `GH_TOKEN`／`GITHUB_TOKEN`，或 `gh auth login` 的 github.com 憑證 | 沒有 `Authorization` 標頭 |
> | **`GH_ENTERPRISE_TOKEN`／`GITHUB_ENTERPRISE_TOKEN`** | **那個 token，有效，就在標頭裡** |
>
> 第二列不是假設的組合——那正是 `docs/deploying.md` 為 GitHub Enterprise Server 指定的路。
> 對那類部署，0.1.0 的這個缺陷是**憑證外洩**，不只是外連重導。所以嚴重性不再是「未定」：
> 對一般 github.com 部署是「外連目的地可控」，對 GHES 部署是「憑證會送出去」。
>
> **是讀的，不是量的**——要量得架一台憑證被信任的 TLS 監聽器。那段程式碼是一個沒有任何設定
> 參與的字串比較，所以這個讀法不脆弱，但它終究是讀來的，而且只釘在那兩個版本上。
